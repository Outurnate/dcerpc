...
fn main()
{
  ...
  library.include("src/ncklib");
  library.include("src/ncklib/include/linux-gnu");
  library.include("src/include");
  library.include(idl_dir.clone());
  library.define("MIA", None);
  library.define("API", None);
  library.define("_POSIX_C_SOURCE", None);
  library.define("DCETHREAD_ENFORCE_API", None);
  library.define("_XOPEN_SOURCE", "500");
  library.define("__EXTENSIONS__", None);
  library.define("_POSIX_PTHREAD_SEMANTICS", None);
  library.define("_ALL_SOURCE", "1");
  library.define("_REENTRANT", None);
  library.define("HAVE_CONFIG_H", None);
  library.define("ATTRIBUTE_UNUSED", "__attribute__((unused))");
  library.define("IMAGE_DIR", "\"/usr/local/lib\"");
  library.define("CATALOG_DIR", "\"/usr/local/share/dce-rpc\"");
  let profile = std::env::var("PROFILE").unwrap();
  if profile.as_str() == "release"
  {
    library.define("_FORTIFY_SOURCE", "2");
  }
